...
class Admin extends Authcheck
{
        ...
        if($this->request->isPost()) {
            ...
            if(empty($post['group_id'])) {
                ...
                try{
                    ...
                    $group_access['group_id'] = $post['group_id'];
                    $group_access['create_time'] = time();
                    $group_access['update_time'] = time();
                    \think\loader::model('common/adminGroupAccess')->save($group_access);
                    // $list = [];
    ...
    public function edit()
    {
        if($this->request->isPost()) 
        {
            ...
            $info['admin_group'] = \think\loader::model('common/adminGroupAccess')->where(['uid'=>$id])->find();
    ...
    public function profile()
    {
        ...
        $data['image'] = $this->domain.$data['avatar'];
        if(empty($data)) 
        {
            return $this->error('非法操作');
        }
