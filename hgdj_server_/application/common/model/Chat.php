...
class Chat extends Model
{
     ...
     * @param $chater_id
     */
    public function scopeUsers($query, $user_id, $chater_id, $chat_id = null)
    {
        $query->where(function ($query) use ($user_id, $chater_id, $chat_id) {
            $query->where('send_uid', $user_id)->where('rec_uid', $chater_id);
            if ($chat_id) {
                $query->where('id', '<', $chat_id);
            }
        })->whereOr(function ($query) use ($user_id, $chater_id, $chat_id) {
    ...
    public function scopeNotReadAll($query, $user_id)
    {
        $query->where('rec_uid', $user_id)->where('is_read', self::STATUS_UNREAD);
    }
}