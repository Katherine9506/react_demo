...
Page({
    ...
    albumTap: function (e) {
        ...
        if (dataset.is_vip) {
            wx.navigateTo({
                url: '/pages/indexList/indexList?album_id=' + dataset.album_id + "&album_title=" + dataset.title,
            });
            return;
        }
        wx.navigateTo({
            url: '/pages/indexList2/indexList2?album_id=' + dataset.album_id + "&album_title=" + dataset.title,
        });
    },
    /* 获取该专辑标签下的专辑列表 */
